...
const CardOrcamentoEstruturas = ({estruturas}) => {
  ...
  return (
    ...
    >
      <CardHeader title={'Estrutura Metálica'} type={'estrutura'}/>
    
      {estruturas.map(({label, quantidade, metragem, valor}, index) => ( 
        <div  key={index} className='gap-0.5 flex flex-col text-slate-100'>
          ...
          <p><strong className='text-fundo-verdeH' >Metragem :</strong> {metragem} m</p>
          <p><strong className='text-red-500' >Valor :</strong> {formataValor(Number((Number(metragem) * Number(valor)) * Number(quantidade) ))}</p>
        </div>
      ))}
...
export default CardOrcamentoEstruturas